...
본 단계에서 확정하는 사항: Hard Filter와 Soft Constraint 구분, 정책·승인·행사·사용자 제외 필터, 가격·MOQ·생산량·공급지역 필터, 유통채널·OEM·PB·수출 조건, 상담시간·일정·거리·접근성 필터, 부스 운영·재고·혼잡 필터, 정보 미등록·미검증 처리, 필터 결과·사유·버전 저장, 조건 완화 제안, 관리자 예외처리, 필터 API·DB·성능지표.

| 단계 | 상세 내용 |
|---|---|
| 제11단계 | 일반 관람객 Soft Score |
| 제12단계 | 바이어 Soft Score |
| 제13단계 | 양면 적합도 |
| 제14단계 | 상황 재정렬 |
| 제15단계 | 공정성·노출정책 |
| 제17단계 | 행동 피드백 |
...
**진행 현황**: 전체 30단계 중 완료 10단계 (진행률 33.3%)

다음 단계는 제11단계 일반 관람객 초개인화 매칭점수 설계로, 방문목적·주종·맛·향·도수·가격·구매조건·체험·거리·혼잡·행동·데이터 신뢰도를 정량화하고 최종 B2C 점수 산식과 보정규칙을 정의한다.
