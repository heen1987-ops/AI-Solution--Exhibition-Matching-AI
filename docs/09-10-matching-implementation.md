...
| `feature_builder.py` | Feature Builder | `CONSUMER_SCORE_V1`(category/price/goal/sensory/alcohol/usage/service)·`BUYER_SCORE_V1`(product/price/moq/business_goal/channel/region/capacity)·`EXHIBITOR_SCORE_V1`(order_volume/buyer_type/channel/region/verification) 구성요소를 채운다. goal/sensory/alcohol/usage/business_goal/channel/region/buyer_type은 `profile_attribute.attribute_code` 접두어(GOAL/TASTE·AROMA/ALCOHOL_LEVEL/USE/BIZ_GOAL/CHANNEL/REGION/BUYER, docs/06-matching-ontology.md 5.1/8~14절)를 `_COMPONENT_BY_CODE_PREFIX`로 해석해 후보 쪽 concept_id 집합(product_attribute/trade_condition_term/buyer_preference)과 교집합 비교한다. capacity는 `exhibition.supply_capability.available_capacity`(잔여 생산능력, trade_condition.monthly_capacity와는 다른 숫자)와 바이어의 월 요구물량을 직접 비교한다(숫자 비교라 concept 매칭이 아니다). service는 고정 코드("SERVICE.TASTING"/"SERVICE.PURCHASE") 문자열을 `exhibition.event_product.tasting_status`/`purchase_status`와 비교한다(concept_id가 아니라 문자열 자체 비교 - 이 두 값만 다루면 되므로). verification은 `exhibition.supply_capability.verification_status`(SELF_DECLARED~VERIFIED 6단계)를 [0,1]로 변환한다 - 매칭 대상이 아니라 후보 자체의 품질 신호라 프로파일 쪽 요구가 필요 없다. cooperation/trade_type/portfolio/decision_timing/meeting_readiness/meeting/behavior/trust는 여전히 다른 데이터(상태값 비교, 포트폴리오, 상담 일정, 신뢰도 모델)가 필요해 `None`이다(모듈 docstring 참고) |
...
- DB 통합테스트(`backend/tests/conftest.py`, `test_orchestrator_integration.py`):
  ...
  - GENERAL_VISITOR 경로(RecommendationSession 1건, MatchResult 1건, `VISIT_NOW`,
    ...
    verification 구성요소 연결)까지 열 시나리오를 검증한다.
...
3. ~~`profile_resolver`가 돌려주는 ProfileAttribute를 온톨로지 concept_type과 함께 해석해
   ...
   tasting_status/purchase_status 문자열 비교로, verification(EXHIBITOR)는 exhibition.
   supply_capability.verification_status를 [0,1]로 변환해 채웠다(위 "서비스 계층" 표
   feature_builder.py 행 참고). cooperation/trade_type/portfolio/decision_timing/
   meeting_readiness/meeting/behavior/trust는 concept·숫자·상태값 비교가 아닌 다른
   데이터(포트폴리오, 상담 일정, 신뢰도 모델)가 필요해 여전히 미구현이다.
