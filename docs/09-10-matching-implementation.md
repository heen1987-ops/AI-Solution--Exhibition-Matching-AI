...
| `feature_builder.py` | Feature Builder | `CONSUMER_SCORE_V1`(category/price/goal/sensory/alcohol/usage/service)·`BUYER_SCORE_V1`(product/price/moq/business_goal/channel/region/capacity/cooperation)·`EXHIBITOR_SCORE_V1`(order_volume/buyer_type/channel/region/verification) 구성요소를 채운다. goal/sensory/alcohol/usage/business_goal/channel/region/buyer_type은 `profile_attribute.attribute_code` 접두어(GOAL/TASTE·AROMA/ALCOHOL_LEVEL/USE/BIZ_GOAL/CHANNEL/REGION/BUYER, docs/06-matching-ontology.md 5.1/8~14절)를 `_COMPONENT_BY_CODE_PREFIX`로 해석해 후보 쪽 concept_id 집합(product_attribute/trade_condition_term/buyer_preference)과 교집합 비교한다. capacity는 `exhibition.supply_capability.available_capacity`(잔여 생산능력, trade_condition.monthly_capacity와는 다른 숫자)와 바이어의 월 요구물량을 직접 비교한다(숫자 비교라 concept 매칭이 아니다). service/cooperation은 고정 코드("SERVICE.TASTING"/"SERVICE.PURCHASE", "TRADE.OEM"/"TRADE.PB"/"TRADE.EXPORT") 문자열을 각각 `exhibition.event_product.tasting_status`/`purchase_status`, `exhibition.trade_condition.oem_status`/`private_label_status`/`export_status`와 비교한다(concept_id가 아니라 문자열 자체 비교 - 다루는 코드 개수가 고정적이라). verification은 `exhibition.supply_capability.verification_status`(SELF_DECLARED~VERIFIED 6단계)를 [0,1]로 변환한다 - 매칭 대상이 아니라 후보 자체의 품질 신호라 프로파일 쪽 요구가 필요 없다. trade_type(EXHIBITOR)은 cooperation과 원본 데이터는 같지만 반대 방향 신호(exhibition.buyer_preference에 거래유형 차원이 없다)가 없어 `None`으로 남긴다. portfolio/decision_timing/meeting_readiness/meeting/behavior/trust는 여전히 다른 데이터(포트폴리오, 상담 일정, 신뢰도 모델)가 필요해 `None`이다(모듈 docstring 참고) |
...
| `context_reranker.py` | 14단계 상황 인지 재정렬 | 기본 적합도(normalized_score)는 그대로 두고 상황 신호로 최종 순위만 다시 매긴다(05단계 5.9절 "기본 적합도와 '지금 방문할 가치'는 분리해 저장한다"). `context_resolver.py`가 주는 세션 공통 신호(남은 체류시간, 다음 확정 일정)에 더해, `exhibition.booth.estimated_wait_minutes`를 참가업체(participation_id)로 조인해 후보별 부스 대기시간까지 반영한다(`orchestrator._booth_wait_minutes_by_recommendable`). 품절 임박·프로그램 시작시간은 재고 이벤트·프로그램 일정 연동이 아직 없어 `ContextSignals`에 훅만 남겼다. 재정렬은 배제가 아니므로 상황이 아무리 나빠도 기본점수의 최소 절반은 남는다(`_CONTEXT_FLOOR`) |
...
- DB 통합테스트(`backend/tests/conftest.py`, `test_orchestrator_integration.py`):
  ...
  - GENERAL_VISITOR 경로(RecommendationSession 1건, MatchResult 1건, `VISIT_NOW`,
    ...
    것(feature_builder의 concept 기반 구성요소 연결), 기여 구성요소마다 MatchReason이
    ...
    raw_score가 높다는 것(BUYER_SCORE_V1의 capacity 구성요소 연결), 그리고 시음을
    요구하는 프로파일에 대해 시음 가능한 후보가 일시중단된 후보보다 raw_score가
    높다는 것(CONSUMER_SCORE_V1의 service 구성요소 연결), 그리고 검증 완료(VERIFIED)
    업체가 자기신고(SELF_DECLARED) 업체보다 raw_score가 높다는 것(EXHIBITOR_SCORE_V1의
    verification 구성요소 연결), 그리고 OEM 생산을 요구하는 바이어에 대해 OEM 가능
    업체가 OEM 불가 업체보다 raw_score가 높다는 것(BUYER_SCORE_V1의 cooperation
    구성요소 연결)까지 열한 시나리오를 검증한다.
...
3. ~~`profile_resolver`가 돌려주는 ProfileAttribute를 온톨로지 concept_type과 함께 해석해
   ...
   available_capacity 숫자 비교로, service(CONSUMER)는 exhibition.event_product의
   tasting_status/purchase_status 문자열 비교로, cooperation(BUYER)은 exhibition.
   trade_condition의 oem_status/private_label_status/export_status 문자열 비교로,
   verification(EXHIBITOR)는 exhibition.supply_capability.verification_status를
   [0,1]로 변환해 채웠다(위 "서비스 계층" 표 feature_builder.py 행 참고). trade_type은
   cooperation과 원본은 같지만 반대 방향 데이터(exhibition.buyer_preference에 거래유형
   차원이 없음)가 없어 의도적으로 비워뒀다. portfolio/decision_timing/meeting_readiness/
   meeting/behavior/trust는 concept·숫자·상태값 비교가 아닌 다른 데이터(포트폴리오,
   상담 일정, 신뢰도 모델)가 필요해 여전히 미구현이다.
...
6. ~~14단계(상황 재정렬)와 18단계(추천 이유 생성)를 오케스트레이터의 해당 자리에
   ...
   남은 체류시간·다음 일정(context_resolver.py)에 더해 부스 대기시간
   (exhibition.booth.estimated_wait_minutes)까지 반영한다. 05단계 5.9절이 언급하는
   품절 임박·프로그램 시작시간은 재고 이벤트·프로그램 일정 연동이 아직 없어 훅으로만
   남아있다. 18단계는 BUYER 경로(`ReciprocalScoreResult`에 contributions가 없다)에는
   아직 연결하지 못했다.
