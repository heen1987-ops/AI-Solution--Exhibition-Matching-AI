...
> 문서 상태: 30단계 통합 설계 로드맵 — 9단계 산출물
> 로드맵: [30단계 통합 설계 로드맵](./00-roadmap.md)
> 선행 계약: [7단계 사용자 프로파일 모델](./07-user-profile-model.md), [8단계 참가업체·제품 프로파일 모델](./08-exhibitor-product-profile-model.md), [6단계 매칭 분류체계·온톨로지](./06-matching-ontology.md). 이 문서의 속성·카테고리 코드 표기는 API·질의 가독성을 위한 코드 표현이며, DB·인덱스에서는 해당 버전의 `(taxonomy_version_id, concept_id)`를 함께 참조한다.

문제를 다시 정의하면, 전체 전시 대상에서 추천 가능성이 있는 후보군을 빠르고 넓게 확보하는 검색 계층을 설계하는 단계다. 이 단계의 목표는 최종 순위를 정하는 것이 아니라, 정답 후보를 빠뜨리지 않으면서 계산 가능한 규모로 줄이는 것이다. 검색 단계에서 후보를 놓치면 뒤의 AI가 아무리 영리해도 없는 후보를 창조할 수는 없다 — 생성형 AI가 가끔 시도하긴 하지만, 그게 바로 환각이다.
...
후보검색은 사용자 원본 프로파일 전체를 직접 사용하지 않고, 검색에 필요한 정보만 정규화한 검색 요청 객체를 사용한다. 프로파일 속성은 7·8단계 계약에 따라 `attribute_code` 표현과 함께 `(taxonomy_version_id, concept_id)`를 보유하지만, 검색 요청 객체 예시에서는 가독성을 위해 코드만 표기한다.
...
실제 스키마에서는 `category_code` 등의 코드 컬럼이 `concept_id`(및 조회 시점의 `taxonomy_version_id`)로 저장되므로, 구조화 검색은 6단계 온톨로지 API·동의어 해석을 거친 뒤 이 코드값으로 질의한다.
...
확장 깊이와 관계가중치는 6단계 온톨로지의 `concept_revision` 계층·버전에 따라 관리한다.
...
단, 복잡한 조건 충족 여부는 10단계 Hard Filter에서 최종 검증한다.
...
막걸리 ↔ 탁주, 전통소주 ↔ 증류식 소주, 바틀샵 ↔ 주류 전문점, 자체브랜드 ↔ PB, 납품 ↔ 공급·정기공급, 달지 않은 ↔ 드라이, 목 넘김 좋은 ↔ 부드러운. 6단계 온톨로지의 동의어 해석기를 우선 사용하고, 형태소 분석으로 보완한다.
...
MOQ, 공급지역, 생산역량, 가격 절대조건, OEM·PB, 수출국, 일정 충돌, 접근성, 남은 시간은 10단계에서 정밀하게 판단한다. 후보검색 단계에서 복잡한 필터를 과도하게 적용하면 재현율이 낮아질 수 있다.
...
인덱스 버전, 온톨로지 버전(`taxonomy_version_id`), 검색문서 템플릿 버전, 임베딩 모델 버전, 생성시간, 대상 데이터 기준시점, 문서 수, 오류 수
...
| matched_codes | 일치 온톨로지 개념(`concept_id`) |
...
1. 후보검색과 최종 추천순위 분리
2. 재현율 우선의 고재현율 후보군 생성
3. 구조화·키워드·벡터·행동·인기·신규 검색 결합
4. 일반 관람객과 바이어 검색계획 분리
5. 사용자 프로파일을 검색 요청 객체로 정규화
6. 검색계획에 채널별 후보 수와 가중치 적용
7. 벡터 검색은 후보 생성에만 사용
8. 검색채널 결과를 정규화한 후 RRF 방식 병합
9. 내부 표준 ID 기준 중복 제거
10. 업체별 제품 후보 수 상한 적용
11. 후보 수 부족 시 단계적 검색범위 확장
12. 필수조건은 자동 완화하지 않음
13. 검색채널 장애 시 부분 성공·대체검색 적용
14. 검색 인덱스·온톨로지·임베딩 모델 버전 저장
15. 제품·업체·부스·프로그램·상담 인덱스 분리
16. B2C·B2B 검색문서와 임베딩 분리
17. 추천 당시 후보검색 결과와 채널별 근거 저장
18. 후보검색 품질을 Recall@K 중심으로 평가

다음 단계는 10단계 필수조건 필터·제외규칙 설계이며, 승인·운영상태·가격·MOQ·공급지역·생산량·일정·접근성·사용자 제외조건을 Hard Filter와 Soft Constraint로 구분하고 제외 사유와 조건 완화 절차까지 정의한다.
