...
from app.services.matching.explanation_generator import EXPLANATION_POLICY_VERSION
...
EXPLANATION_VERSION = EXPLANATION_POLICY_VERSION
...
def _validate_candidates(candidates: list[MatchCandidate]) -> None:
    ...
    for candidate in candidates:
        ...
        if not candidate.cold_start_policy_version:
            raise _contract_error("콜드스타트 정책 버전이 없습니다.")
        if not candidate.cold_start_input_fingerprint:
            raise _contract_error("콜드스타트 계산 지문이 없습니다.")
        ...
        if not candidate.reasons:
            raise _contract_error("모든 추천 결과에는 최소 한 개의 추천 이유가 필요합니다.")
        for reason in candidate.reasons:
            if reason.explanation_policy_version != EXPLANATION_POLICY_VERSION:
                raise _contract_error("추천 이유 정책 버전이 현재 게시 버전과 다릅니다.")
            if not reason.input_fingerprint or len(reason.input_fingerprint) != 64:
                raise _contract_error("추천 이유 입력 지문이 없습니다.")
            if not reason.evidence_refs:
                raise _contract_error("추천 이유에는 최소 한 개의 근거 참조가 필요합니다.")
    ...
    *,
    ...
    policy_versions = {
        ...
        for version in (
            ...
            candidate.cold_start_policy_version,
    ...
    if not trace.cold_start_policy_version or not trace.cold_start_input_fingerprint:
        raise _contract_error("콜드스타트 실행 계보가 없습니다.")
    cold_start_policy = policies[trace.cold_start_policy_version]
    ...
    match_run = MatchRun(
        ...
        cold_start_policy_version_id=cold_start_policy.match_policy_version_id,
        cold_start_details={
            "policy_version": trace.cold_start_policy_version,
            "input_fingerprint": trace.cold_start_input_fingerprint,
            **trace.cold_start_metrics,
        },
        context_snapshot={
    ...
    for candidate in candidates:
        ...
        cold_policy = policies[candidate.cold_start_policy_version]
        match_result = MatchResult(
            ...
            cold_start_policy_version_id=cold_policy.match_policy_version_id,
            ...
            cold_start_details={
                "policy_version": candidate.cold_start_policy_version,
                "state": candidate.cold_start_state,
                "quality_score": candidate.cold_start_quality_score,
                "reason_codes": list(candidate.cold_start_reason_codes),
                "input_fingerprint": candidate.cold_start_input_fingerprint,
            },
        ...
        for reason in candidate.reasons:
            db.add(
                MatchReason(
                    ...
                    explanation_policy_version=reason.explanation_policy_version,
                    input_fingerprint=reason.input_fingerprint,
