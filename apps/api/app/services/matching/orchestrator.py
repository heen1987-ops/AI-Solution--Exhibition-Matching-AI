...
class RecommendationOrchestrator:
    ...
    async def generate(
        ...
        *,
        ...
        # ⑨-1 Stage 16: cold-start confidence/exploration overlay.  It does not
        # alter relevance and therefore runs after scoring, before slate assembly.
        step_start = time.perf_counter()
        cold_start = await cold_start_runtime.evaluate_and_annotate(
            db,
            subject=validated.subject,
            profile=profile,
            candidates=eligible,
        )
        timings["cold_start_policy"] = _elapsed_ms(step_start)
        ...
        trace = PipelineTrace(
            ...
            cold_start_policy_version=cold_start.policy_version,
            cold_start_input_fingerprint=cold_start.input_fingerprint,
            cold_start_metrics={
                "state": cold_start.state,
                "type_codes": list(cold_start.type_codes),
                "stability_score": cold_start.stability_score,
                "exploration_ratio": cold_start.exploration_ratio,
                "recommendation_confidence_cap": cold_start.recommendation_confidence_cap,
                "next_question_codes": list(cold_start.next_question_codes),
            },
        )
