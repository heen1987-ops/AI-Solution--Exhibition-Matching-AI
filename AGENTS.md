...
3개 모듈: (A) 웹 초개인화 모듈(사전등록 일반·바이어 대상), (B) 키오스크 이식형 검색모듈(현장 비등록·익명 게스트 대상), (C) 공통 AI·데이터 플랫폼. 단일 기준 문서는 `docs/redesign-v2/00-master-spec-v1.md`다 - 다른 어떤 채팅 기록·외부 링크보다 이 문서와 그 하위 `docs/redesign-v2/web|kiosk|common/*.md`, `04-integration-roadmap.md`가 우선한다(`.harness/assumptions.md` ASSUMPTION-004 - 구 버전 AGENTS.md가 참조하던 외부 ChatGPT 스레드는 이 문서 체계로 대체됨).
...
- 키오스크에 회원가입·전화번호·이메일 입력·장기 사용자 프로파일을 추가하지 않는다.
- 키오스크 세션 종료 후 개인정보가 잔존하지 않는다(`docs/redesign-v2/kiosk/K-1-service-scope.md` §1, `K-8-*.md` §4).
...
계획만 반복하고 구현하지 않는 행위, 이미 정해진 사항을 다시 질문하는 행위, 범위 밖 기능을 "좋은 아이디어"라는 이유로 구현하는 행위, 테스트 실패 무시, 다른 트랙 소유 파일 무단 수정, AI 출력값 미검증 저장, 키오스크 개인정보 입력 추가, 상담 수락 전 연락처 공개, 미승인 업체의 검색·추천 포함, `다음` 입력 시 전체 계획만 다시 설명하는 행위.

## 12. 배포 (기존 프로젝트 관행 - 계속 유지)

구현 단위가 관련 검증을 통과하면 의도적으로 커밋하고, 사용자가 달리 말하지 않는 한 `exhibition` git remote로 푸시한다. 비밀값·로컬 의존성 디렉터리·무관한 작업공간 변경을 포함하지 않는다.
